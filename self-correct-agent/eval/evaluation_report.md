...
|---|---|---:|---|---:|---:|---:|
| GSM8K | 无（模型自评） | 1 | `self_refine` | 90.0% | 0 | 4 |
| GSM8K | 无（模型自评） | 2 | `self_refine_r2` | 92.0% | 2 | 4 |
...
- 无外部信号的 Self-Refine（1 轮）为 90%，相对 CoT 改对 0 题、改错 4 题。模型会把听起来合理的自我批评当成事实，因此可能越改越差。
...
轮数对照为：数学 CoT 0 轮 94%、Self-Refine 1 轮 90%、2 轮 92.0%；代码 Direct 0 轮 82%、单测修复 1 轮 85%、2 轮 85.0%。这支持“失败才修改、通过就停止”的门控策略；单纯增加轮数没有带来单调收益。
